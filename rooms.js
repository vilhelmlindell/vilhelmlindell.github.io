...
async function getFreeRooms() {
    ...
    for (let room_lesson_times of lessonTimes) {
        ...
        for (let i = 0; i < room_lesson_times[1][day].length; i++) {
            ...
            if (current_time <= lesson_time[0]) {
                ...
                if (i === room_lesson_times[1][day].length - 1) {
                ...
                let free_duration;
                if (i < room_lesson_times[1][day].length - 1) {
                    let next_lesson_time = room_lesson_times[1][day][i + 1];
    ...
    freeRoomsElement.innerHTML = '<tr><th>Lediga Rum Spyken</th><th>Tid Ledigt</th></tr>' + free_rooms.map(room => `<tr class="free"><td>${room[0]}</td><td>${room[1]}</td></tr>`).join('');
